  ...
  useEffect(() => {
    const fetchFlashDeals = async () => {
      try {
        ...
        // Filter only items with originalPrice > price
        const flashDeals = dishesData.filter((item: any) => item.originalPrice && item.originalPrice > item.price);
        setDishes(flashDeals);
      } catch (error) {
    ...
    <div className="bg-[#f8f9fa] font-body text-on-surface min-h-screen pb-32 md:pb-12">
      ...
      <header className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-gray-100">
        <div className="flex justify-between items-center px-6 md:px-8 h-16 md:h-20 max-w-7xl mx-auto">
          ...
          <nav className="hidden md:flex items-center gap-8">
            ...
            <Link to="/flash-deals" className="text-primary font-bold border-b-2 border-primary pb-1 text-sm">Flash Deals</Link>
            <Link to="/orders" className="text-gray-500 hover:text-primary transition-colors font-medium text-sm">Orders</Link>
            <Link to="/favorites" className="text-gray-500 hover:text-primary transition-colors font-medium text-sm">Favorites</Link>
          </nav>
      ...
      <main className="mt-20 md:mt-28 px-4 md:px-8 max-w-7xl mx-auto space-y-6 md:space-y-8">
        <section className="space-y-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              ...
              <p className="text-on-surface-variant mt-2 max-w-2xl text-xs md:text-sm">
                Up to 40% OFF today. Grab these deals before they are gone!
              </p>
            </div>
          ...
          {loading ? (
            <div className="text-center py-12">Loading flash deals...</div>
              ...
              {filteredDishes.map(item => {
                ...
                <Link key={item.id} to={`/dish/${item.id}`} className="bg-surface-container-lowest rounded-2xl md:rounded-3xl overflow-hidden flex flex-col shadow-sm hover:shadow-xl transition-all duration-300 group border border-outline-variant/20 relative h-[240px] md:h-[280px]">
                  <div className="relative h-[160px] md:h-[186px] overflow-hidden flex-shrink-0">
                    ...
                    {hasDiscount && (
                      <div className="absolute top-2 left-2 md:top-2.5 md:left-2.5 bg-primary/95 backdrop-blur-md px-1.5 py-0.5 md:px-2 md:py-0.5 rounded-full flex items-center gap-0.5 shadow-sm">
                        <span className="text-[8px] md:text-[9px] font-extrabold text-on-primary">{discountPercent}% OFF</span>
                      </div>
                    )}
